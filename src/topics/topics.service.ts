...
export class TopicsService {
    ...
    try {
      ...
      if (!savedTopic) {
        throw new ManagerError({ 
          type: 'BAD_REQUEST', message: 'Error al crear un tema' 
        });
      }
      return savedTopic;
    } catch (error) {
  ...
  async findOne(id: string) {
    try {
      const topic = await this.topicRepo.findOne({ where: { id }, relations: { subject: true } });
      if (!topic) throw new ManagerError({ type: 'NOT_FOUND', message: 'Tema no encontrado' });
      return topic;
    } catch (error) {
